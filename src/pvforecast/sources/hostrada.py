...
from .base import DownloadError, HistoricalSource, WeatherSourceError
...
class HOSTRADASource(HistoricalSource):
    ...
    def fetch_historical(self, start: date, end: date) -> pd.DataFrame:
        ...
        downloads = [(param, year, month) for param in parameters for year, month in months]
        ...
        # Log summary
        if error_count > 0:
            logger.info(f"✓ {success_count}/{total_files} files loaded ({error_count} unavailable)")
        else:
    ...
    def _estimate_dhi(self, ghi: np.ndarray, times: pd.DatetimeIndex) -> np.ndarray:
        ...
        sin_alt = np.sin(lat_rad) * np.sin(decl_rad) + np.cos(lat_rad) * np.cos(decl_rad) * np.cos(
            ha_rad
        )
